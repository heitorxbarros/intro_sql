...
.small-code[
```{sql eval = F}
SELECT DISTINCT NUMFUNC, NOME, LENGTH(NOME)
...
.small-code[
```{sql eval = F}
SELECT DISTINCT MES_ANO_FOLHA, SUBSTR(MES_ANO_DIREITO, 4) MES_ANO, SUBSTR(MES_ANO_DIREITO, 4, 2) MES
...
.small-code[
```{sql eval = F}
SELECT DISTINCT MES_ANO_FOLHA, 'ANO: ' || SUBSTR(MES_ANO_DIREITO, 7) AS MES
FROM ERGON.FICHAS_FINANCEIRAS
WHERE MES_ANO_FOLHA IN ('01/01/2023','01/02/2023','01/03/2023','01/04/2023');
```
]
...
.small-code[
```{sql eval = F}
SELECT DISTINCT NUMFUNC, DTAPOSENT, DTVAC, COALESCE(DTAPOSENT,DTVAC,'09/11/2023')            
FROM ERGON.VINCULOS 
WHERE ROWNUM <= 10000;
```
]
...
.small-code[
```{sql eval = F}
SELECT DISTINCT RUBRICA, MEAN(VALOR) MEDIA, ROUND(MEAN(VALOR), 2) MEDIA_ARR          
FROM ERGON.FICHAS_FINANCEIRAS 
...
.small-code[
```{sql eval = F}
SELECT DISTINCT RUBRICA, ABS(VALOR)          
FROM ERGON.FICHAS_FINANCEIRAS 
